...
# 🆕 VERSIÓN ACTUALIZADA A v7.9.2 - Blindaje Definitivo de Migración de Columnas
st.set_page_config(page_title="Control Económico Familiar v7.9.2 Cloud", page_icon="💰", layout="wide")
...
def init_db():
    with engine.begin() as conn:
        ...
        # 🛡️ FORZAR ADICIÓN DE COLUMNA 'cuenta' SI LA TABLA YA EXISTÍA SIN ELLA
        try:
            ...
            print("Info migración movimientos:", e)
        ...
        try:
            ...
            print("Info migración meses_cerrados:", e)
        ...
        try:
            ...
            print("Info primary key meses_cerrados:", e)
...
def limpiar_duplicados_df(df_mov):
    ...
    res = []
    lista_cuentas = df_mov['cuenta'].unique() if 'cuenta' in df_mov.columns else ['operativa']
...
def obtener_metricas_ahorro_completa(anio, saldo_inicial):
    # Consulta segura comprobando si existe la columna cuenta en la base de datos antes de seleccionarla
    try:
...
st.title("💰 Control Económico Familiar v7.9.2 Cloud")
...
try:
    ...
    try:
        ...
        meses_cerrados_df['cuenta'] = 'operativa'
    except:
            ...
            try:
                if uploaded_bank.name.endswith(".csv"): df_banco = pd.read_csv(uploaded_bank)
                    ...
                    for r_i in range(min(20, len(df_raw))):
                        ...
                        if any("IMPORTE" in v or "CANTIDAD" in v for v in row_vals): header_row = r_i; break
                ...
                if st.button("🚀 Importar y Conciliar en esta Cuenta", type="primary") and col_imp and col_desc and col_fec:
                    try:
                        ...
                        movs_existentes['cuenta'] = 'operativa'
                        ...
                        if importe_val != 0:
                            ...
                            if pd.notna(raw_fec):
                                ...
                                if isinstance(raw_fec, (datetime, pd.Timestamp)): f_dt = pd.to_datetime(raw_fec)
                                    ...
                                    else: f_dt = pd.to_datetime(f_str_temp, dayfirst=True, errors='coerce')
                                if f_dt and pd.notna(f_dt):
                                    f_str = f_dt.strftime("%Y-%m-%d")
                                    if "Ciclo de Nómina" in modo_importacion: a_dest, m_dest = anio_sel, st.session_state.mes_seleccionado
                                    else: a_dest, m_dest = f_dt.year, MESES_MAPPING_NUM.get(f_dt.month, m_dest)
            ...
            with st.expander(f"{tag} | **{concepto}** | {total_concepto:,.2f} € | *({num_movs} movimientos)*"):
                if not has_real and len(df_c) == 1:
                    ...
                    with st.form(f"f_p_ing_{r_prev_item['id']}"):
                        cq1, cq2, cq3 = st.columns([3, 2, 2])
                        cq1.write("Ajuste rápido de previsión:")
                        nuevo_imp_val = cq2.number_input("Imp", value=float(r_prev_item['importe']), step=10.0, label_visibility="collapsed")
                        if cq3.form_submit_button("💾 Guardar"):
                            with engine.begin() as conn:
                                conn.execute(text("UPDATE movimientos SET importe = :imp WHERE id = :id"), {"imp": float(nuevo_imp_val), "id": int(r_prev_item['id'])})
                            st.rerun()
                    st.markdown("---")
                ...
                for _, row in df_mostrar.iterrows():
                    ...
                    if c_act.button("✏️ Cambiar", key=f"edit_mov_{row['id']}"):
                        st.session_state.enseñar_id = row['id']; st.session_state.vista_anterior = 'MENSUAL'; st.session_state.vista_nivel = 'ENSENAR_REGLA'; st.rerun()
            ...
            if blk == "COMIDA":
                ...
                with st.expander(f"{tag} | **Alimentación (Total del mes)** | {total_concepto:,.2f} € | *({num_movs} movimientos)*"):
                    if not has_real and len(df_b) == 1:
                        ...
                        with st.form(f"f_p_com_{r_prev_item['id']}"):
                            cq1, cq2, cq3 = st.columns([3, 2, 2])
                            cq1.write("Ajuste rápido de previsión:")
                            nuevo_imp_val = cq2.number_input("Imp", value=float(r_prev_item['importe']), step=10.0, label_visibility="collapsed")
                            if cq3.form_submit_button("💾 Guardar"):
                ...
                for concepto in df_b['concepto'].unique():
                    ...
                    with st.expander(f"{tag} | **{concepto}** | {total_concepto:,.2f} € | *({num_movs} movimientos)*"):
                        if not has_real and len(df_c) == 1:
                            r_prev_item = df_c.iloc[0]
                            with st.form(f"f_p_c_{r_prev_item['id']}"):
                                cq1, cq2, cq3 = st.columns([3, 2, 2])
                                cq1.write("Ajuste rápido de previsión:")
                                nuevo_imp_val = cq2.number_input("Imp", value=float(r_prev_item['importe']), step=10.0, label_visibility="collapsed")
                                if cq3.form_submit_button("💾 Guardar"):
                        ...
                        if st.button(f"🔍 Opciones Avanzadas (Añadir Tickets a {concepto})", key=f"btn_det_{blk}_{concepto}"):
                            st.session_state.detalle_concepto = concepto; st.session_state.detalle_bloque = blk; st.session_state.vista_nivel = 'DETALLE_AGRUPADO'; st.rerun()
                ...
                if c_final:
                    with engine.begin() as conn:
                        for mid in selected_ids_mes:
                            conn.execute(text("UPDATE movimientos SET bloque = :b, concepto = :c, tipo = :t WHERE id = :id"), {"b": b_mas, "c": c_final.strip(), "t": t_mas, "id": mid})
...
# ==========================================
    ...
    if "REGLA" in ambito:
        condicion_regla = st.radio("¿Cómo debe detectar la regla los siguientes movimientos?", ["1) Solo por Palabra Clave (Ej: MERCADONA)", f"2) Por Palabra Clave + IMPORTE EXACTO ({nuevo_importe:,.2f} €)"])
        ...
        if patron and len(patron.strip()) >= 2:
            ...
            if not df_conflicto.empty:
                ...
                if regla_ant['bloque'] != bloque_in or regla_ant['concepto'] != concepto_in:
                    st.warning(f"⚠️ **¡OJO, CONFLICTO DETECTADO!**\n\nYa tienes una regla para **'{patron.strip()}'** hacia **{regla_ant['bloque']} ➔ {regla_ant['concepto']}**.\nSi guardas, la sobrescribirás.")
    
    if st.button("💾 Guardar Cambios", type="primary"):
        if concepto_in:
            with engine.begin() as conn:
                conn.execute(text("UPDATE movimientos SET bloque = :b, concepto = :c, tipo = :t, importe = :imp, descripcion_original = :desc WHERE id = :id"), {"b": bloque_in, "c": concepto_in.strip(), "t": tipo_val, "imp": float(nuevo_importe), "desc": nueva_descripcion.strip(), "id": int(mov['id'])})
                ...
                if "REGLA" in ambito and patron and len(patron.strip()) >= 2:
                    ...
                    if not df_existe.empty:
                        conn.execute(text("UPDATE reglas_categorias SET bloque = :b, concepto = :c, importe_exacto = :i WHERE id = :id"), {"b": bloque_in, "c": concepto_in.strip(), "i": imp_exacto_val, "id": int(df_existe.iloc[0]['id'])})
                    else:
                        conn.execute(text("INSERT INTO reglas_categorias (patron, bloque, concepto, importe_exacto) VALUES (:p, :b, :c, :i)"), {"p": str(patron.strip()), "b": bloque_in, "c": concepto_in.strip(), "i": imp_exacto_val})
            ...
            time.sleep(1); st.session_state.enseñar_id = None; st.session_state.vista_nivel = st.session_state.vista_anterior; st.rerun()
        else: st.error("Indica un nombre de grupo válido.")
